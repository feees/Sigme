...
/**
 * Stateless session bean implementing the "Manage Spiritist" use case
 * component. See the implemented interface documentation for details.
 * 
...
public class ManageSpiritistsServiceBean extends CrudServiceBean<Spiritist> implements SpiritistsService {
	...
	public void authorize() {
		// Overridden to implement authorization. @RolesAllowed is placed in the whole
		// class.
	}
	...
	public void validateCreate(Spiritist entity) throws CrudException {
		...
		String crudExceptionMessage = "The spiritist \"" + entity.getName() + "(" + email
				+ ")\" cannot be created due to validation errors.";
			...
			try {
				...
				if (anotherEntity != null) {
					logger.log(Level.INFO,
							"Creation of spiritist \"{0}\" violates validation rule 1: spiritist with id {1} has same email",
							new Object[] { email, anotherEntity.getId() });
					crudException = addValidationError(crudException, crudExceptionMessage, "email",
							"spiritist.error.repeatedEmail", anotherEntity.getLastUpdateDate());
				}
			} catch (PersistentObjectNotFoundException e) {
				logger.log(Level.FINE, "Rule 1 OK - there's no other spiritist with the same email: {0}", email);
			} catch (MultiplePersistentObjectsFoundException e) {
				logger.log(Level.WARNING, "Creation of spiritist with email \"" + email
						+ "\" threw an exception: a query for spiritists with this email returned multiple results!",
						e);
				crudException = addValidationError(crudException, crudExceptionMessage, "name",
						"spiritist.error.multipleInstancesError");
			}
	...
	public void validateUpdate(Spiritist entity) throws CrudException {
		...
		CrudException crudException = null;
		String email = entity.getEmail();
		String crudExceptionMessage = "The spiritist \"" + email + "(" + email
				+ ")\" cannot be updated due to validation errors.";
		...
		if (email != null && email.length() > 0) {
			try {
				...
				if ((anotherEntity != null) && (!anotherEntity.getId().equals(entity.getId()))) {
					logger.log(Level.INFO,
							"Update of spiritist \"{0}\" violates validation rule 1: spiritist with id {1} has same email",
							new Object[] { email, anotherEntity.getId() });
					crudException = addValidationError(crudException, crudExceptionMessage, "email",
							"spiritist.error.repeatedEmail", anotherEntity.getLastUpdateDate());
				}
	...
	protected void log(CrudOperation operation, Spiritist entity) {
		...
		case CREATE:
		case UPDATE:
			logger.log(Level.FINER, "Storing spiritist: {0} / {1} / {2} ...",
					new Object[] { entity.getName(), entity.getEmail(), entity.getTaxCode() });
			logger.log(Level.FINER,
					"Persisting data...\n\t- Birthdate = {0}\n\t- Gender = {1}\n\t- Short name = {2}\n\t- Address = {3}\n\t- Telephones = {4}\n\t- {5} attendances",
					new Object[] { entity.getBirthDate(), entity.getGender(), entity.getShortName(),
							entity.getAddress(), entity.getTelephones(), entity.getAttendances().size() });
			break;
		case RETRIEVE:
			logger.log(Level.FINE, "Loaded spiritist with id {0} ({1})...",
					new Object[] { entity.getId(), entity.getEmail() });
			break;
		case DELETE:
			logger.log(Level.FINE, "Deleted spiritist with id {0} ({1})...",
					new Object[] { entity.getId(), entity.getEmail() });
			break;
		default:
	...
	protected void log(CrudOperation operation, List<Spiritist> entities, int... interval) {
		switch (operation) {
		case LIST:
			logger.log(Level.FINE, "Retrieved spiritists in interval [{0}, {1}): {2} spiritist(s) loaded.",
					new Object[] { interval[0], interval[1], entities.size() });
			break;
		default:
			logger.log(Level.WARNING, "CRUD bean received unknown operation logging: {0}", new Object[] { operation });
		}
