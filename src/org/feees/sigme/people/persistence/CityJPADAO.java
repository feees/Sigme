...
import org.feees.sigme.people.domain.City_;
import org.feees.sigme.people.domain.State;
import org.feees.sigme.people.domain.State_;
 ...
 * @see org.feees.sigme.people.persistence.CityDAO
 */
@Stateless
public class CityJPADAO extends BaseJPADAO<City> implements CityDAO {
	...
	/** @see org.feees.sigme.people.persistence.CityDAO#findByName(java.lang.String) */
	@Override
	public List<City> findByName(String name) {
		...
		cq.where(cb.like(cb.lower(root.get(City_.name)), name.toLowerCase() + "%"));
		cq.orderBy(cb.asc(root.get(City_.name)));
	...
	/** @see org.feees.sigme.people.persistence.CityDAO#retrieveByNameAndStateAcronym(java.lang.String, java.lang.String) */
	@Override
	public City retrieveByNameAndStateAcronym(String name, String stateAcronym) throws PersistentObjectNotFoundException, MultiplePersistentObjectsFoundException {
		...
		CriteriaQuery<City> cq = cb.createQuery(City.class);
		Root<City> root = cq.from(City.class);
		Join<City, State> join = root.join(City_.state);

		// Filters the query with the name and state acronym.
		cq.where(cb.equal(root.get(City_.name), name), cb.equal(join.get(State_.acronym), stateAcronym));
